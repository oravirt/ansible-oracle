...
options:
    ...
    sqlselect:
        description: >
            ...
            To access the column "value" of the first row
            write: "<<registered result>>.state.ROW[0].VALUE" (use uppercase).
        required: false
        default: None
    creates_sql:
        description: >
            This is the check query to ensure idempotence..
            Must be a single SQL select that results to no rows or a plain 0 if the
            catcon_pl/sqlscript/sql has to be executed. Any other result prevent the
            execution of catcon_pl/sqlscript/sql..
            The catcon_pl/sqlscript/sql will be executed unconditionally if creates_sql
            is omitted..
            Creates_sql must be omitted when sqlselect is used..
            Creates_sql is executed with sqlplus / as sysdba in the root container.
            Write the sql query according to this fact..
            If pdb_list is given (implicitely whith all_pdbs) creates_sql is executed
            in every PDB incl. CDB$ROOT. The pdb_list will be shortened according to
            the results of creates_sql in the PDBs..
        required: false
        default: None
    username:
        description: Database username, defaults to "/ as sysdba"
        required: false
        default: None
    password:
        ...
        default: None
    scope:
        description: Shall the SQL be applied to CDB, PDBs, or both?
        values:
    ...
    pdb_list:
        description: >
            ...
            Will be automatically filled and used when scope = all_pdbs and action
            like sql%.
        required: false
        default: None
    oracle_home:
        description: content of $ORACLE_HOME
    oracle_db_name:
        description: SID or DB_NAME, needed for BEQ connect
    nls_lang:
...
- name: Conditionally execute post installation tasks
  oracle_sqldba:
    ...
    username: >
        {{ pitask.username | default(omit) }}
    password: >
        {%if pitask.username is defined%}
        {{ dbpasswords[oradb.oracle_db_name][pitask.username] }}
        {% endif %}
    scope: "{{ pitask.scope | default(omit) }}"
    pdb_list: >
        {{ oracle_pdbs | default([])
            | json_query('[?cdb==`' + oradb.oracle_db_name + '`].pdb_name')
            | join(' ') }}
