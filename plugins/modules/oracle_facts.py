...
def main():
    ...
    try:
        rac = query_result(
            "SELECT inst_id, instance_name, host_name, startup_time "
            "FROM gv$instance "
            "ORDER BY inst_id"
        )
    ...
    try:
        if database['CDB'] == 'YES':
            pdb = query_result(
                "SELECT con_id, rawtohex(guid) guid_hex, name, open_mode "
                "FROM v$pdbs "
                "ORDER BY name"
            )
        else:
    ...
    if conn.version >= '12.1':
        tablespace = query_result(
            "select ts.con_id, ts.name, ts.bigfile, "
            "       round(sum(bytes)/1024/1024) size_mb, count(*) datafiles# "
        ...
        temp_tablespace = query_result(
            "select ts.con_id, ts.name, ts.bigfile, "
            "round(sum(bytes)/1024/1024) size_mb, count(*) tempfiles# "
            "from v$tablespace ts "
            "join v$tempfile df "
            "     on df.ts#=ts.ts# "
            "     and df.con_id=ts.con_id "
        ...
        tablespace = query_result(
            "select 0 con_id, ts.name, ts.bigfile, "
            "round(sum(bytes)/1024/1024) size_mb, count(*) datafiles# "
        ...
        temp_tablespace = query_result(
            "select 0 con_id, ts.name, "
            "ts.bigfile, round(sum(bytes)/1024/1024) size_mb, count(*) tempfiles# "
            "from v$tablespace ts join v$tempfile df on df.ts#=ts.ts# "
            "group by ts.name, ts.bigfile "
            "order by 1,2"
        )
    redolog = query_result(
        "select group#, thread#, sequence#, round(bytes/1024/1024) mb, "
        "       blocksize, archived, status "
        "from v$log "
    ...
    if conn.version >= '11.1':
        sql += (
            ", to_number(sys_context('USERENV','CURRENT_EDITION_ID')) "
            "    CURRENT_EDITION_ID, "
