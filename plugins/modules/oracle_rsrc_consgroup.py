...
description:
    ...
    - For more accurate documentation about attributes, please check Oracle DB
      documentation about DBMS_RESOURCE_MANAGER
version_added: "2.2.1"
options:
    hostname:
    ...
    state:
        description: >
            If present, then consumer group is created,
            if absent, then consumer group is removed
        required: true
        choices: ['present','absent']
    name:
        description: Resource consumer group name
        required: True
    comments:
        description: Comment about the group
        required: False
    mgmt_mth:
        description: Name of CPU resource allocation method
        default: round-robin
    category:
        description: Describes the category of the consumer group
        default: other
    map_client_id:
        description: >
            This procedure adds, deletes, or modifies entries that map sessions to
            consumer groups, based on the session's login and runtime attributes.
            Check Oracle documentation on
            DBMS_RESOURCE_MANAGER.SET_CONSUMER_GROUP_MAPPING on more details.
        required: False
        type: list
    map_client_machine:
        description: >
    ...
    map_oracle_user_profile:
        description: >
            Module appends all users with mentioned profiles to map_oracle_user
            parameter list
        required: False
        type: list
    map_service_module:
...
def query_existing(name):
    ...
    if c.rowcount > 0:
        ...
        c.execute(
            (
                "SELECT attribute, LISTAGG(value,':') WITHIN GROUP (ORDER BY value) "
...
def profile_list_to_users(profiles):
    ...
    c = conn.cursor()
    c.execute(
        "SELECT username FROM dba_users WHERE profile IN (%s)" % ",".join(inlist),
...
def main():
    ...
    module = AnsibleModule(
        argument_spec=dict(
            ...
            grant_name=dict(
                required=False,
                type='list',
                aliases=['grant', 'grant_user', 'grant_role', 'grants'],
            ),
    ...
    if module.params['state'] == 'present':
        new_grants = new_grants_list(
            module.params['grant_name'], module.params['grant_user_profile']
        )
        new_mappings = new_mappings_dict()
    if result['exists'] and module.params['state'] == "present":
        # Check attributes and modify if needed
        if (
            (result['comments'] != module.params['comments'])
            or (result['mgmt_mth'] != module.params['mgmt_mth'].upper())
            or (result['category'] != module.params['category'].upper())
            or (new_grants != result['grants'])
            or (new_mappings != result['mappings'])
        ):
            c = conn.cursor()
            ...
            for map_attr in new_mappings.keys():
                ...
                t = (
                    result['mappings'][map_attr]
                    if map_attr in result['mappings']
                    else set()
                )
            ...
            BEGIN
                ...
                FOR i IN 1..v_add_grants.COUNT LOOP
                    DBMS_RESOURCE_MANAGER_PRIVS.GRANT_SWITCH_CONSUMER_GROUP(
                        grantee_name=>v_add_grants(i),
                        consumer_group=>v_name,
                        grant_option=>false
                        );
                END LOOP;
                FOR i IN 1..v_remove_grants.COUNT LOOP
                    DBMS_RESOURCE_MANAGER_PRIVS.REVOKE_SWITCH_CONSUMER_GROUP(
                        revokee_name=>v_remove_grants(i),
                        consumer_group=>v_name
                        );
                END LOOP;
            END;""",
        ...
        END;
        """,
            {"name": module.params['name'], "grants": var_grants},
        )
        result_changed = True
